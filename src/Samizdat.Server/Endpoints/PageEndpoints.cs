...
public static class PageEndpoints
{
    ...
    public static RouteGroupBuilder MapPages(this WebApplication app)
    {
                                 ...
                                 IAntiforgery antiforgery, HttpContext context, Translator text) =>
        {
            ...
            // Своя ячейка, а не роль и язык в ключе: с общей ячейкой владелец и читатель, как и
            // двое с разными языками, вытесняли бы страницы друг друга, и каждый второй запрос
            // шёл бы в полный рендер.
            var cell = $"{text.Code}/{(isOwner ? slug : $"reader/{slug}")}";
